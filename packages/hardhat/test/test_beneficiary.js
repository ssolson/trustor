...
describe("🚩 🏵 Simple Trust 🤖", () => {
  ...
  describe("Claim Pro Rata", () => {
    it("claim: Correct Shares Recieved", async () => {
      const { wallets, simpleT } = await loadFixture(fixture2);
      ...
      const total_beneficiaries_0 = await simpleT.getBeneficiariesLength();
      console.log("total_beneficiaries_0", total_beneficiaries_0);
      ...
      // Beneficiary 2 Claims
      const totalShares = await simpleT.totalShares();
      const tokenBal_Trust_0 = await simpleT.balanceOf(
        simpleT.address,
        simpleT._tokenIds(wallets["Grantor2"].address)
      );
      const Beneficiary2 = wallets["Beneficiary2"];
      ...
      const tokenBal_Trust_1 = await simpleT.balanceOf(
        simpleT.address,
        simpleT._tokenIds(wallets["Grantor2"].address)
      );
      const tokenBal_Beneficiary2_1 = await simpleT.balanceOf(
      ...
      const tokenBal_Beneficiary3_0 = await simpleT.balanceOf(
        Beneficiary3.address,
        simpleT._tokenIds(wallets["Grantor2"].address)
      );
      ...
      const shares_Beneficiary3 = await simpleT.beneficiaryShares(
        Beneficiary3.address
      );

      await simpleT.connect(Beneficiary3).claim();

      const tokenBal_Trust_2 = await simpleT.balanceOf(
        simpleT.address,
        simpleT._tokenIds(wallets["Grantor2"].address)
      );
      const tokenBal_Beneficiary3_1 = await simpleT.balanceOf(
        Beneficiary3.address,
        simpleT._tokenIds(wallets["Grantor2"].address)
      );
