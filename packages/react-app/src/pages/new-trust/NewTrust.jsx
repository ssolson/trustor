          ...
          <Form.Item>
            <Button
              ...
              onClick={async () => {
                ...
                // const Grantors = vals['grantorAddresses']['address'];
                let grantors = vals['grantorAddresses'];
                // let grantor_names = grantors.map(element => element.name);
                let Grantors = grantors.map((element) => element.address);

                // const initial_trustee_name = grantors[0]["name"];
                const initial_trustee_addr = vals['initialTrusteeWallet'];
                ...
                let SuccessorTrustees = trustees.map((element) => element.address);
                ...
                const CheckInPeriod = vals['checkInPeriod'];

                // let argz = [Name, Grantor, Trustee, Beneficiary, Shares];
                let argz = [
                  Name,
                  initial_trustee_addr,
                  CheckInPeriod,
                  Grantors,
                  SuccessorTrustees,
                ...
                const newTrustDoc = {
                  ...
                  grantor_address: Grantors,
                  trustee_address: SuccessorTrustees,
                ...
                addresses.forEach(async (address, idx) => {
                  ...
                  let response = await fetch(`http://localhost:5000/user/${address}`).catch(
                    (error) => {
                      // window.alert(error);
                      console.log(error);
                      return;
                  ...
                  if (!record) {
                    ...
                    console.log(
                      'record[new_trust_address] === undefined',
                      record[new_trust_address] === undefined
                    );
